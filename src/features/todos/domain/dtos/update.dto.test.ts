...
describe('tests in update.dto.test.ts', () => {
	...
	test('should throw a validation error for an invalid id', () => {
		expect(() => UpdateTodoDto.create({ id: NaN, isCompleted: true })).toThrow(ValidationError);
		expect(() => UpdateTodoDto.create({ id: null, isCompleted: true })).toThrow(ValidationError);
		expect(() => UpdateTodoDto.create({ id: undefined, isCompleted: true })).toThrow(ValidationError);
		expect(() => UpdateTodoDto.create({ id: 'invalid', isCompleted: true })).toThrow(ValidationError);
	});

	test('should throw a validation error for an invalid isCompleted (not a boolean)', () => {
		expect(() => UpdateTodoDto.create({ id: 1, isCompleted: 'invalid' })).toThrow(ValidationError);
	});
